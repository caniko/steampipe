...
/// A confirmation handler that supports **in-session retry**.
///
/// Unlike [`AuthConfirmationHandler`], which is consulted once, this handler is
/// re-consulted by
/// [`login_with_retry`](crate::connection::UnAuthenticatedConnection::login_with_retry)
/// whenever a submitted code is rejected, so a corrected code can be resubmitted
/// on the *same* auth session. Restarting authentication instead would issue a
/// fresh email Steam Guard code, which is exactly what we want to avoid. Added
/// in the steampipe vendor fork.
pub trait RetryConfirmationHandler {
    /// Provide the next Steam Guard code, or `None` to abort the login.
    ///
    /// `previous_error` is `Some` when an earlier attempt on this session was
    /// rejected, letting the implementation surface it and re-prompt.
    fn next_code(
        &mut self,
        allowed_confirmations: &[ConfirmationMethod],
