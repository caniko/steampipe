...
impl SteamGuardToken {
    /// Construct a guard token from a raw one-time code (email or device code).
    ///
    /// Added in the steampipe vendor fork to support in-session retry: a
    /// rejected code can be resubmitted on the same auth session without
    /// restarting authentication (which would mail a fresh email code).
...
impl PendingAuth {
    pub(crate) async fn wait_for_tokens(
        ...
        loop {
            ...
            // The poll returned a non-token status. `poll_until_info` short-
            // circuits on any populated field — including `had_remote_interaction`,
            // which becomes sticky once a guard code has been submitted — so it
            // can return immediately without ever sleeping. Pace this outer loop
            // at Steam's recommended interval so an unapproved session (notably a
            // *rejected* guard code, which `login_with_retry` drives on purpose
            // until its timeout) does not hammer `PollAuthSessionStatus` at the
            // network round-trip rate and risk auth rate-limiting. A correct code
            // returns the access token on the first poll and never reaches here.
