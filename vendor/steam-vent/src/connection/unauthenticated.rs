...
use crate::auth::{
    AuthConfirmationHandler, ConfirmationAction, GuardDataStore, RetryConfirmationHandler,
    SteamGuardToken, begin_password_auth,
};
...
/// the attempt as rejected and re-prompting (steampipe vendor fork).
///
/// A correct code makes the next poll return tokens (well within one poll
/// interval); a wrong code is silently accepted by the submit RPC but never
/// yields tokens, so it manifests only as the absence of a response. Bounding
/// the wait turns that silence into a retry signal. Kept generous so a correct
/// code on a slow link is never misread as a rejection.
...
impl UnAuthenticatedConnection {
    ...
    /// Like [`login`](Self::login), but supports **in-session retry** of the
    /// Steam Guard code via a [`RetryConfirmationHandler`].
    ///
    /// Authentication is begun exactly once (so the email code is mailed once),
    /// then the submit/poll cycle loops: each code is submitted on the same
    /// session and we poll for tokens under [`RETRY_POLL_TIMEOUT_SECS`]. A
    /// correct code returns tokens; a rejected code never does (the submit RPC
    /// accepts it silently), surfacing as a timeout we translate into a retry by
    /// re-consulting the handler with the failure reason. Returning `None` from
    /// the handler aborts with [`ConnectionError::Aborted`]. Added in the
    /// steampipe vendor fork.
    pub async fn login_with_retry<H: RetryConfirmationHandler, G: GuardDataStore>(
        self,
        account: &str,
        password: &str,
        mut guard_data_store: G,
        ...
        let allowed_confirmations = begin.allowed_confirmations();
        let needs_code = allowed_confirmations
            .iter()
            .any(|method| method.token_type().is_some());

        let tokens = if needs_code {
            let mut previous_error: Option<String> = None;
            loop {
                let Some(code) = handler
                    .next_code(&allowed_confirmations, previous_error.as_deref())
                    .await
                else {
                    return Err(ConnectionError::Aborted);
                };
                let Some(token_type) = allowed_confirmations
                    .iter()
                    .find_map(|method| method.token_type())
                else {
                    return Err(ConnectionError::UnsupportedConfirmationAction(
                        allowed_confirmations.clone(),
                    ));
                ...
                if code.is_empty() {
                    previous_error = Some("the Steam Guard code was empty".to_string());
                    continue;
                }
                ...
                begin
                    .submit_confirmation(
                        &raw,
                        ConfirmationAction::GuardToken(SteamGuardToken::new(code), token_type),
                    )
                    .await?;

                match timeout(
                    Duration::from_secs(RETRY_POLL_TIMEOUT_SECS),
                    begin.poll().wait_for_tokens(&raw),
                )
                .await
                {
                    Ok(Ok(tokens)) => break tokens,
                    Ok(Err(e)) => previous_error = Some(format!("login error: {e}")),
                    Err(_) => {
                        previous_error =
                            Some("the Steam Guard code was rejected or expired".to_string());
                    }
                }
            }
        } else if begin.action_required() {
            ...
            begin.poll().wait_for_tokens(&raw).await?
        };
        ...
        raw.session = login(
            ...
            account,
            steam_id,
            // refresh token sent as access token — required by the protocol
            tokens.refresh_token.as_ref(),
        )
        .await?;
        raw.setup_heartbeat();
