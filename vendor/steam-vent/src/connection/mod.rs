...
use crate::auth::{AuthConfirmationHandler, GuardDataStore, RetryConfirmationHandler};
use crate::message::{
...
impl Connection {
    ...
    /// Like [`login`](Self::login), but supports in-session retry of the Steam
    /// Guard code via a [`RetryConfirmationHandler`]. Added in the steampipe
    /// vendor fork; see
    /// [`UnAuthenticatedConnection::login_with_retry`].
    pub async fn login_with_retry<H: RetryConfirmationHandler, G: GuardDataStore>(
        ...
        UnAuthenticatedConnection::connect(server_list)
            .await?
            .login_with_retry(account, password, guard_data_store, handler)
            .await
    }
