...
| Prior headless Steam login lifecycle work | Retired from published planning. | Implementation landed in working tree: virtiofsd lifecycle in `src/core/backend.rs` + `src/core/state.rs`, kvm-group warning in `nix/module.nix`, parallel `steam login` + reuse-running-VM in `src/game/steam.rs`. | **Shipped/superseded.** Durable behavior is now documented in [Steam Credentials](../configuration/steam-credentials.md) and [Global host config](../configuration/host-config.md). |
