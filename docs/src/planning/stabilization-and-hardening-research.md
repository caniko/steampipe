...
| `headless-steam-login/` (3 phases + research dossier) | Added in working tree (not yet in HEAD). | Implementation landed in working tree: virtiofsd lifecycle in `src/core/backend.rs` + `src/core/state.rs`, kvm-group warning in `nix/module.nix`, parallel `steam login` + reuse-running-VM in `src/game/steam.rs`. | **Shipped (pending verify).** Plan files at [headless-steam-login/](./headless-steam-login/); dossier at [headless-steam-login-research.md](./headless-steam-login-research.md). Phase set covers the original `cluster-ctl steam login all` failure (`Failed to connect to 'vm-N-virtiofs-steam-state-vm-N.sock'`) by spawning per-VM virtiofsd before QEMU. Real-host validation on the atlas cluster is the remaining acceptance step. |
